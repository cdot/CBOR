...
export default class Decoder {
  ...
  readIndefiniteBytes(majorType) {
    ...
    while ((length = readChunkLength()) >= 0) {
      ...
      elements.push(this.stream.readBytes(length));
    }
  ...
  decodeItem() {
    ...
    case 0: // unsigned integer
      if (this.debug) this.debug(`${this.stream.mark}: UINT ${ai}`);
      if (ai === 31)
        throw Error("Invalid 0 AI");
      return this.readArgument(ai);

    case 1: // negative integer
      if (this.debug) this.debug(`${this.stream.mark}: -INT ${ai}`);
      if (ai === 31)
        throw Error("Invalid 1 AI");
      return -1 - this.readArgument(ai);

    case 2: // byte string
      if (this.debug) this.debug(`${this.stream.mark}: BYTES ${ai}`);
      if (ai === 31)
        return this.readIndefiniteBytes(majorType);
      return this.stream.readBytes(this.readArgument(ai));

    case 3: // UTF-8 encoded text string
      if (this.debug) this.debug(`${this.stream.mark}: TEXT ${ai}`);
      if (ai === 31)
        return new TextDecoder().decode(this.readIndefiniteBytes(majorType));
      return new TextDecoder().decode(this.stream.readBytes(this.readArgument(ai)));

    case 4: // array of data items
      if (ai === 31) {
        if (this.debug) this.debug(`${this.stream.mark}: ARRAY?`);
        return this.readIndefiniteItemArray();
      }
      len = this.readArgument(ai);
      if (this.debug) this.debug(`${this.stream.mark}: ARRAY ${len}`);
      return this.readItemArray(len);

    case 5: // map of pairs of data items
      if (ai === 31) {
        if (this.debug) this.debug(`${this.stream.mark}: MAP?`);
        return this.readIndefiniteKV();
      }
      len = this.readArgument(ai);
      if (this.debug) this.debug(`${this.stream.mark}: MAP length ${len}`);
    ...
    case 6: // tagged data item
      ...
      if (this.debug) this.debug(`${this.stream.mark}: TAG ${tag}`);
    ...
    case 7: // floating point number and values with no content
      ...
      if (this.debug) this.debug(`${this.stream.mark}: OTHER ${ai}`);
      switch (ai) {
      case 20:
        return false;
      case 21:
