...
class TagHandler {
  ...
  /**
   * Where trailing data is found in the input stream, this method
   * will be called on each such item. It's really only for handling
   * old KeyDictionary data, which used to be output at the end of
   * data without a tag. New handlers should always tag data.
   * @param {object} id key identifier, as generated by `encodeKey`
   * @param {Decoder} decoder the calling decoder
   * @return {string} the actual key
   */
  trailingData(decoder, data) { } // ignore
}
